...
def save_updated_series_to_pickle(updated_series, filename):
    with open(filename, 'wb') as file:
        pickle.dump(updated_series, file)
...
def generate_random_values(ideal_ohm, tolerance_percentage, num_components):
    ...
    return [random.uniform(min_value, max_value) for _ in range(num_components)]

def generate_charts_per_series(df):
    #TODO: Make Charts Work
  """Generates charts per series, grouping together counts of simulated values based on ideal values.

  Args:
    df: A Pandas DataFrame with ideal value, tolerance percentage, series, and simulated values.
  """

  # Create a temporary dataframe that counts occurrences of simulated values
  temp_df = df.copy()
  for i in range(1, len(temp_df.columns) - 2):
    temp_df[f"Sim_{i}"] = temp_df[f"Sim_{i}"].astype("category")
  temp_df = temp_df.groupby(["Ideal Ohm", "Series"]).agg(count=("Sim_1", "nunique"))

  # Generate charts for each series
  for series in temp_df["Series"].unique():
    plt.figure(figsize=(10, 6))
    temp_df[temp_df["Series"] == series].plot.bar(x="Ideal Ohm", y="count")
    plt.xlabel("Ideal Ohm")
    plt.ylabel("Count")
    plt.title(f"Count of Simulated Values for {series} Series")
    plt.show()
    

def get_random_value(df, ideal_ohm_value):
    # Get the row corresponding to the ideal ohm value
    row = df[df["Ideal Ohm"] == ideal_ohm_value]

    # Access the "Random Values" column and select a random value
    random_values_list = row["Random Values"].values[0]
...
if __name__ == "__main__":
    ...
    num_simulated_components_per_ideal = 1000  # You can change this as needed
    ...
    for series in updated_e_series:
        for ideal_value in series['values']:
            ...
            data.append({
                ...
                "Series": series['name'],
                **{f"Sim_{i}": val for i, val in enumerate(random_values, start=1)},
                "Random Values": random_values  # Save random values as a list
            })


    # Create a DataFrame with ideal value as the index
    df = pd.DataFrame(data).set_index("Ideal Ohm")
    ...
    # Print the DataFrame
    print(df)
    # generate_charts_per_series(df)
    save_updated_series_to_pickle(updated_e_series,"updated_e_series.pkl")
    random_value = get_random_value(df, 220)
    print (random_value)
    # Show the filename
