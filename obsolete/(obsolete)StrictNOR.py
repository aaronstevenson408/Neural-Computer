...
class NORGateDataset(Dataset):
    def __init__(self, num_points, data_file):
        self.num_points = num_points
        self.data_file = data_file
        self.data = self.load_data() if os.path.exists(self.data_file) else self.generate_data()
...
class DeepNORModel(nn.Module):
    def __init__(self, input_size):
        super(DeepNORModel, self).__init__()
        # Define the layers
        self.fc1 = nn.Linear(input_size, 64)  # Add a hidden layer with 64 neurons
        self.relu = nn.ReLU()  # Add a ReLU activation function
        self.fc2 = nn.Linear(64, 1)  # Output size is 1 for binary classification
...
if os.path.exists(data_file):
    ...
    custom_dataset = NORGateDataset(num_data_points, data_file)
